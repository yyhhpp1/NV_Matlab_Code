...
% gScope.visa=visa('ni','GPIB0::26::INSTR');
% 
% 
% 
% 
...
% fopen(gScope.visa);
% 
% fprintf(gScope.visa,sprintf('*RST'));
% fprintf(gScope.visa,sprintf('TIMEBASE:RANGE 1E-6'));
% fprintf(gScope.visa,sprintf('CHANNEL1:RANGE 3.2'));
% fprintf(gScope.visa,sprintf('TRIGger:EDGE:SOURce EXTernal'));
% fprintf(gScope.visa,sprintf('ACQUIRE:TYPE AVERAGE'));
% fprintf(gScope.visa,sprintf('ACQUIRE:COMPLETE 100'));
% fprintf(gScope.visa,sprintf('WAVEFORM:SOURCE CHANNEL1'));
% fprintf(gScope.visa,sprintf('WAVEFORM:FORMAT WORD'));
% fprintf(gScope.visa,sprintf('ACQUIRE:COUNT 4'));
% fprintf(gScope.visa,sprintf('WAVEFORM:POINTS 500'));
% fprintf(gScope.visa,sprintf('DIGITIZE CHANNEL1'));
% A= query(gScope.visa,'WAVEFORM:DATA?');
% fclose(gScope.visa);
% msg=double(A(11:end));
% plot(msg)
...
try
    ...
    fprintf(visaObj,'*RST'); 
    fprintf(visaObj,sprintf('TIMEBASE:RANGE 1E-6'));
    fprintf(visaObj,sprintf('CHANNEL1:RANGE 3.2'));
    fprintf(visaObj,sprintf('TRIGger:EDGE:SOURce EXTernal'));
    fprintf(visaObj,sprintf('TRIGger:EDGE:LEVel .4'));
    fprintf(visaObj,sprintf('TRIGger:EDGE:COUPling AC'));
    ...
    fprintf(visaObj,':ACQUIRE:COUNT 10000');
    % Specify 5000 points at a time by :WAV:DATA?
    %fprintf(visaObj,':WAV:POINTS:MODE RAW'); % this is undefined on this scope
    fprintf(visaObj,':WAV:POINTS MAX');
...
%% Helper functions
function instrumentError=err(visaObj)

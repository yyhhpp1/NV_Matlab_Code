...
for iStep = 1:numel(cfg.stepOrder)
    ...
    if ~strcmpi(stepCfg.kind, 'preset')
        ...
        capture_step_fit_outputs(stepCfg);
...
seq.fixFreq = resolve_derived_value('gmSEQ.ESRFitFreq_m1_GHz');
...
seq.TO2 = num2str(to2);
seq.SweepNPoints2 = get_control_string(h, 'nPtsT1');
end

function seq = build_step_t00_2(h)
[from1, to1, from2, to2] = split_sweep_range( ...
...
seq.fixPow = get_control_string(h, 'MWPowerRabi');
seq.fixPow2 = get_control_string(h, 'MWPowerRabi2');
seq.fixFreq = resolve_derived_value('gmSEQ.ESRFitFreq_p1_GHz');
...
seq.FROM2 = num2str(from2);
seq.TO2 = num2str(to2);
seq.SweepNPoints2 = get_control_string(h, 'nPtsT12');
end

function seq = build_step_t11(h)
[from1, to1, from2, to2] = split_sweep_range( ...
    str2double(get_control_string(h, 'startT13')), ...
    str2double(get_control_string(h, 'stopT13')));

seq.name = 'T1_S11_S1m1';
seq.FROM1 = num2str(from1);
seq.TO1 = num2str(to1);
seq.SweepNPoints = get_control_string(h, 'nPtsT13');
seq.fixPow = get_control_string(h, 'MWPowerRabi');
seq.fixPow2 = get_control_string(h, 'MWPowerRabi');
seq.fixFreq = resolve_derived_value('gmSEQ.ESRFitFreq_m1_GHz');
seq.fixFreq2 = resolve_derived_value('gmSEQ.ESRFitFreq_p1_GHz');
seq.Repeat = get_control_string(h, 'RepeatT13');
seq.Average = get_control_string(h, 'maxAveT13');
seq.useSG2 = 1;

seq.bSweep2 = 1;
seq.FROM2 = num2str(from2);
seq.TO2 = num2str(to2);
seq.SweepNPoints2 = get_control_string(h, 'nPtsT13');
end
...
switch token
    ...
    case 'gmSEQ.ESRFitFreq_m1_GHz'
        out = sprintf('%.8f', require_gm_field(gmSEQ, 'ESRFitFreq_m1_MHz')/1000);
    case 'gmSEQ.ESRFitFreq_p1_GHz'
        out = sprintf('%.8f', require_gm_field(gmSEQ, 'ESRFitFreq_p1_MHz')/1000);
...
function value = require_gm_field(gmSEQ, fieldName)
if ~isfield(gmSEQ, fieldName) || isempty(gmSEQ.(fieldName)) || ~isfinite(gmSEQ.(fieldName))
    error('AutoPipelineConfig:MissingDerivedSource', ...
        'Required gmSEQ field "%s" is missing or invalid.', fieldName);
end
value = gmSEQ.(fieldName);
end
...
function capture_step_fit_outputs(stepCfg)
global gmSEQ

if ~isfield(stepCfg, 'saveFitFreqTo') || isempty(stepCfg.saveFitFreqTo)
    return;
end

if ~isfield(gmSEQ, 'ESRFitFreq') || isempty(gmSEQ.ESRFitFreq) || ~isfinite(gmSEQ.ESRFitFreq)
    warning('T1_SemiAuto_Program:FitCaptureMissingSource', ...
        'Cannot capture ESR fit frequency for step because gmSEQ.ESRFitFreq is missing.');
    return;
end

targetField = normalize_to_char(stepCfg.saveFitFreqTo);
gmSEQ.(targetField) = gmSEQ.ESRFitFreq;
end

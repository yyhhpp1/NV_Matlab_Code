function PlotT1DataOnlyCharge(handles,raw_j)
...
labels = ["S_{0}","S_{-1}","S_{1}", "Ref_B","Ref_D"];
...
data1 = sig0 ./ refB0;    % S0
data2 = sigm1 ./ refBm1;  % Sm1 
data3 = sig1 ./ refB1;    % S1

ref_err = 1./sqrt(gmSEQ.iAverage * abs(ref0));
sig_err = 1./sqrt(gmSEQ.iAverage * abs(sig0));
rel_err = sqrt(ref_err.^2 + sig_err.^2);
data1_err = rel_err .* data1;

ref_err = 1./sqrt(gmSEQ.iAverage * abs(refm1));
sig_err = 1./sqrt(gmSEQ.iAverage * abs(sigm1));
rel_err = sqrt(ref_err.^2 + sig_err.^2);
data2_err = rel_err .* data2;

ref_err = 1./sqrt(gmSEQ.iAverage * abs(ref1));
sig_err = 1./sqrt(gmSEQ.iAverage * abs(sig1));
rel_err = sqrt(ref_err.^2 + sig_err.^2);

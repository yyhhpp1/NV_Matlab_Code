...
gImageCorr.RefIMG = 'C:\MATLAB_Code\ImageCorrelation\Image_2025-6-25_Img006.txt';
...
bGo = 1;
set(handles.minVx,'String',num2str(gScan.FixVx-20*(gImageCorr.RefRangeVx(2)-gImageCorr.RefRangeVx(1))/(gImageCorr.RefNVx-1)))
set(handles.maxVx,'String',num2str(gScan.FixVx+20*(gImageCorr.RefRangeVx(2)-gImageCorr.RefRangeVx(1))/(gImageCorr.RefNVx-1)))
set(handles.NVx,'String',num2str(41))
set(handles.minVy,'String',num2str(gScan.FixVy-20*(gImageCorr.RefRangeVy(2)-gImageCorr.RefRangeVy(1))/(gImageCorr.RefNVy-1)))
set(handles.maxVy,'String',num2str(gScan.FixVy+20*(gImageCorr.RefRangeVy(2)-gImageCorr.RefRangeVy(1))/(gImageCorr.RefNVy-1)))
set(handles.NVy,'String',num2str(41))
...
% This function recenter the scan range by center to the newly chose cursor
% position (either by clicking on the plot or by change Fix Vx/Vy in the GUI)

%get current scan region in Volt
Vxlim = get(handles.axes1, 'XLim')*gConfocal.Vx_per_um;
Vylim = get(handles.axes1, 'YLim')*gConfocal.Vy_per_um;

dVx = Vxlim(2) - Vxlim(1);
dVy = Vylim(2) - Vylim(1);

%reset scan range
gScan.minVx = gScan.FixVx - dVx/2;
gScan.maxVx = gScan.FixVx + dVx/2;
gScan.minVy = gScan.FixVy - dVy/2;
gScan.maxVy = gScan.FixVy + dVy/2;
...
elseif planScan.ND==2
    ...
    switch mode
        case 'Quick'
            ...
            xlabel(handles.axes1,'distance [um]');
            ylabel(handles.axes1,'distance [um]');
            xlim(handles.axes1, [min(planScan.ContRange*multiplier(1)), max(planScan.ContRange*multiplier(1))])
            ylim(handles.axes1, [min(planScan.FLRange*multiplier(2)), max(planScan.FLRange*multiplier(2))])

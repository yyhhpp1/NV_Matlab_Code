...
switch what
    ...
    case 'Stop'
        Stop(handles)
    case 'Stop After Round'
        Stop_After_Round(handles)
    case 'DC_p'
        AWG_p_Duty_Cycle(handles)
    otherwise
end
end
...
visa_vendor = 'ni';
visa_address = 'TCPIP::169.254.174.198::INSTR';
buffer = 10^8;%2 * 1024;
...
fscanf(awg,'%s');
fwrite(awg, 'SOUR1:ROSC:SOUR EXTERNAL;FREQ 10MHZ;*ESR?');
fscanf(awg,'%s');
...
now = clock;%Benefit of choosing clock components for the name is that after
%12am the files are NOT saved under a new name. This way, data files for a
%scan that runs past 12am will all stay grouped together in the proper
%order.
...
gSavePlots.file.SigRef = ['_Sig_Ref_Plot_' date '.fig'];
gSavePlots.file.SigOverRef = ['_Sig_Over_Ref_Plot_' date '.fig'];
...
% Populate Parameters
fullPath = 'C:\MATLAB_Code\Experiment_Yao\';
file=strcat(fullPath,'ParameterSave.txt');
fid=fopen(file,'r') ;%Write over txt file to preserve last scan and position
fn=fieldnames(handles)';
for str = fn %For each fieldname, see if Style: 'edit' is a sub-field
    fn2=fieldnames(handles.(char(str)));
    idx=isempty(find(strcmp(fn2, 'Style'),1));
    if ~idx
        if strcmp(handles.(char(str)).Style,'edit') %if it is edit, find its string from the file
            %Thankfully, handles always lists its fieldnames in the same
            %order, so printing to the file will be in the same order as reading
            %from the file
            handles.(char(str)).String=fscanf(fid, [char(str) 'value = %s']);
            fscanf(fid,'\n');
        end
    end
end
...
fullPath = 'C:\MATLAB_Code\Experiment_Yao\';
file=strcat(fullPath,'ParameterSave.txt');
fid=fopen(file,'wt') ;%Write over txt file to preserve last scan and position
fn=fieldnames(handles)';
for str = fn %For each fieldname, see if Style: 'edit' is a sub-field
    fn2=fieldnames(handles.(char(str)));
    idx=isempty(find(strcmp(fn2, 'Style'),1));
    if ~idx
        if strcmp(handles.(char(str)).Style,'edit') %if it is edit, print its string to the file
            fprintf(fid, [char(str) 'value = %s\n'], handles.(char(str)).String);
        end
    end
end
fclose(fid);
...
%Which sequence to sweep
 ...
 for val=dataParameters.Sweep_Parameters.vector
     ...
     elseif strfind(sweepP,'Voltage_I')
         handles.(char(sweepP)).String = val;
     elseif strfind(sweepP,'Voltage_Q')
         handles.(char(sweepP)).String = val;
     else
...
global Sent Seq samples len DC_off Pi_Pulse_Calibrate data dataParameters DC Amp Detector awg buffer countsExport bGo sg
...
TheoreticalThreshold = 0; %A reference data point should be around 30k counts for 10^6 measurements
...
WriteSG(sg, 1,dataParameters.Pi_Pulse_Calibrate.freq, str2double(handles.MW_Power.String))
...
while i <= length(dataParameters.Pi_Pulse_Calibrate.vector) && bGo
    ...
    [signal, reference]=InitializeDAQSeq(); %Initialize DAQ In Sequence Mode
    ...
    try
        ...
        while running==2 && bGo
            ...
            if isempty(running)
                ...
                running = 2;
                if warningVISA >=3
                    DAQmxStopClearTask(signal);
                    DAQmxStopClearTask(reference);
        ...
        while running==2 && bGo
            ...
            if isempty(running)
                ...
                running = 2;
                if warningVISA >=3
                    warning('Communication with AWG lost')
                    error('Communication with AWG lost')
                end
                pause(0.005)
            end
    ...
    ancillaSig(i) = sigDatum ;
    ancillaRef(i) = refDatum ;
    
    %Prototype edit 9/14/16
    if i==1
        TheoreticalThreshold = 0.9*ancillaRef(i); %Testing new theoretical threshold scheme
    end
    %End edit
    ...
    if ancillaRef(i)>TheoreticalThreshold
        ...
        warning=0;
        AWG_p_Duty_Cycle(handles) %Need to switch back to a usable sequence
    end
...
global Sent Seq samples len DC_off Rabi data dataParameters DC Amp Detector awg buffer countsExport bGo sg
...
WriteSG(sg, 1, dataParameters.Rabi.freq, str2double(handles.MW_Power.String))
...
len.Rabi = 15; %length of waveform in us

loop_Standard = str2double(handles.Loop_Standard.String) ;
loop_Count = str2double(handles.Loop_Count.String) ;
iteration_Repetitions = str2double(handles.Iteration_Repeats.String) ;
if loop_Count>65000
   loop_Count=65000;
   disp('Loop Counts too high. Set to 65000')
end
...
TheoreticalThreshold = 0.9*(300*10^-9)*loop_Standard*loop_Count*iteration_Repetitions*Count; %A reference data point should be around 30k counts for 10^6 measurements
...
while i<=length(dataParameters.Rabi.vector) && bGo
    try
        ...
        disp(strcat('Iteration', num2str(i), 'of', num2str(length(dataParameters.Rabi.vector))))
        for k=1:iteration_Repetitions %Repeat this iteration
            disp(strcat('Iteration repetition', num2str(k), 'of', num2str(iteration_Repetitions)))
            
            [signal, reference]=InitializeDAQSeq(); %Initialize DAQ In Sequence Mode
            
            RunAWG(1); %Run Sequence (DAQ collects)
            runCount=0;
            running=2;
            tic
            pause(loop_Standard*loop_Count*len.Rabi*10^-6+0.002)
            while running==2 && bGo
                fprintf(awg, '%s\n','AWGCONTROL:RSTATE?');
                running=str2double(fscanf(awg,'%s'));
                if isempty(running)
                    warningVISA=warningVISA+1
                    running = 2;
                    if warningVISA >=3
...
%             end
    ...
    if failed
        warning=warning+1;
        if warning>4
            error('5 errors in a row. Experiment Halted')
        end
    else
        data.Rabi.signal(i)=data.Rabi.signal(i)+ancillaSig(i);
        data.Rabi.reference(i)=data.Rabi.reference(i)+ancillaRef(i);
        warning=0;
        i=i+1;
    end
...
global Sent Seq samples len DC_off ESR data dataParameters DC Amp Detector awg buffer countsExport bGo bGoR sg
...
dataParameters.ESR.rounds = str2double(handles.Rounds.String) ;
dataParameters.ESR.power=str2double(handles.ESR_MWPower.String) ;
DT=.5;
N=2;
dataParameters.ESR.TimeOut=DT*N*1.1;

if dataParameters.ESR.power>-10
    disp('Warning: ESR power too high. Set to -10dBm')
...
rounds=dataParameters.ESR.rounds;
...
AWG_Measure_ESR(MWLength,handles); 
...
%Edit 10/7/2014 Thomas Mittiga--Try turning on MW before tracking and add
%round structure
freq = floor(dataParameters.ESR.vector(1));
WriteSG(sg, 1, freq, dataParameters.ESR.power)
...
for k = 1:rounds
    ...
    disp(['Round ' num2str(k) ' of ' num2str(rounds)])
    %TrackNV(hObject, eventdata)
    Count = countsExport; %Counts per second Update from ImageNVC
    TheoreticalThreshold = 0; %A reference data point should be around 30k counts for 10^6 measurements
    pause(2) %Let z-axis settle
%End Edit

    while i <= length(dataParameters.ESR.vector) && bGo
        disp(['Iteration ' num2str(i) ' of ' num2str(length(dataParameters.ESR.vector))])
        freq = floor(dataParameters.ESR.vector(i));
        WriteSG(sg, 1,freq, dataParameters.ESR.power)
        
        read=0;
        [read,~]=DAQmxFunctionPool('GetCounts',1/DT,N);
...
global Sent Seq samples len DC_off ODMR data dataParameters DC Amp Detector awg buffer countsExport bGo sg
...
WriteSG(sg, 1,dataParameters.ODMR.min, dataParameters.ODMR.MWPower);
%%%input length to Wave_generator
...
while i<=length(dataParameters.ODMR.vector) && bGo
    try
        ...
        WriteSG(sg, 1,freq, dataParameters.ODMR.MWPower);
        
        disp(strcat('Iteration', num2str(i), 'of', num2str(length(dataParameters.ODMR.vector))))
        for k=1:iteration_Repetitions %Repeat this iteration
            disp(strcat('Iteration repetition', num2str(k), 'of', num2str(iteration_Repetitions)))
            ...
            running=2;
            tic
            pause(loop_Standard*loop_Count*len.ODMR*10^-6+0.001)
            while running==2 && bGo
                fprintf(awg, '%s\n','AWGCONTROL:RSTATE?');
                running=str2double(fscanf(awg,'%s'));
                if isempty(running)
...
%                 TrackNV(hObject,eventdata);
%             end
            CreateSeq(loop_Standard,loop_Count,1,'ODMR');
%             error('Reference Counts Low')
        end
    catch
        if ~isempty(signal)
    ...
    if failed
        ...
        data.ODMR.signal(i)=data.ODMR.signal(i)+ancillaSig(i);
        data.ODMR.reference(i)=data.ODMR.reference(i)+ancillaRef(i);
        warning=0;
        i=i+1;
    end
...
global Sent Seq samples len DC_off Ramsey data dataParameters DC Amp Detector awg buffer countsExport bGo bGoR yn sg
...
rounds=dataParameters.Ramsey.rounds;

WriteSG(sg, 1,dataParameters.Ramsey.freq, str2double(handles.MW_Power.String))
...
%user.
if loop_Standard*loop_Count*iteration_Repetitions*dataParameters.Ramsey.rounds>10^6
   tooManyIterations
   waitfor(1010)
   if ~yn
       'Abort measurement'
...
elseif loop_Standard*loop_Count*iteration_Repetitions == 0
    error('No iterations: Remember Loop Standard is divided by rounds and then floor-rounded')
end
...
for j = 1:rounds
    ...
    while i<=length(dataParameters.Ramsey.vector) && bGo
        try
            ancillaSig(i)=0; %Ancillas used as a temperary data storage
            ancillaRef(i)=0; %for all repetitions of an iteration.
            failed=false;
...
global Sent Seq samples len DC_off SpinEcho data dataParameters DC Amp Detector awg buffer countsExport bGo bGoR yn sg
...
WriteSG(sg, 1,dataParameters.SpinEcho.freq, str2double(handles.MW_Power.String))

%In each round, measure each tau loop_Standard*loop_Count*iteration_Repetitions times.
%After iteration_Repetitions measurements of a single iteration of tau,
%check that the NV is still there. it_Rep seems redundant, but it is meant
%to prevent the AOM from cooling down too much.
loop_Standard = floor(str2double(handles.Loop_Standard.String)/rounds) ; 
loop_Count = str2double(handles.Loop_Count.String) ;
iteration_Repetitions = str2double(handles.Iteration_Repeats.String) ;
if loop_Count>65000
   loop_Count=65000;
...
%If loop_Standard and loop_Count are higher than usual, check with the
%user.
if rounds*loop_Standard*loop_Count*iteration_Repetitions>10^6
   tooManyIterations
   waitfor(1010)
   if ~yn
       'Abort measurement'
...
global Sent Seq samples len DC_off CPMGN CPMG data dataParameters DC Amp Detector awg buffer countsExport bGo bGoR yn
...
TrackNV(hObject, eventdata)
Count = countsExport; %1.08*10^5; %Counts per second Update from ImageNVC
TheoreticalThreshold = 0.8*(300*10^-9)*loop_Standard*loop_Count*iteration_Repetitions*Count; %A reference data point should be around 30k counts for 10^6 measurements
...
global Sent Seq samples len DC_off CPMGT CPMG data dataParameters DC Amp Detector awg buffer countsExport bGo bGoR yn
...
dataParameters.CPMGT.n=str2double(handles.CPMGT_n.String) ;
...
%Note that at this point, AWG_p_Duty_Cycle(handles) is still running
TrackNV(hObject, eventdata)
Count = countsExport; %1.08*10^5; %Counts per second Update from ImageNVC
TheoreticalThreshold = 0.6*(300*10^-9)*loop_Standard*loop_Count*iteration_Repetitions*Count; %A reference data point should be around 30k counts for 10^6 measurements
...
data.CPMGT.roundsignal=ancillaSig;
data.CPMGT.roundreference=ancillaRef;
...
for j = 1:rounds
    ...
    while i<=length(dataParameters.CPMGT.vector) && bGo
        ...
        try
            ...
            len.CPMG = max(ceil((4154+(n+1)*piPulse+n*2*tau+2000+300+3000)/1000), ceil((n+1)*piPulse/1000 * 10)); % Guarantee max 5% duty cycle for MW to avoid thermal heating. 10/8/16 by Chong

            % len.CPMG = ceil((4154+(n+1)*piPulse+n*2*tau+2000+300+3000)/1000); %length of waveform in us. Added 3us after reference trigger ends
            disp(' ')
            ...
            for k=1:iteration_Repetitions %Repeat this iteration
                ...
                running=2;
                tic
                pause(loop_Standard*loop_Count*len.CPMG*10^-6+0.001)
                while running==2 && bGo
                    fprintf(awg, '%s\n','AWGCONTROL:RSTATE?');
                    running=str2double(fscanf(awg,'%s'));
                    if isempty(running)
            ...
            if (ancillaSig(i)/ancillaRef(i)>1.2 || ancillaSig(i)/ancillaRef(i)<0.6)
                error('Spike or Dip anomaly')
            end
            
            %Prototype edit 9/14/16
            if i==1
                TheoreticalThreshold = 0.8*ancillaRef(i); %Testing new theoretical threshold scheme
            end
            %End edit
        ...
        if failed
            warning=warning+1;
            if warning>10
                error('10 errors in a row. Experiment Halted')
            end
        else
            data.CPMGT.signal(i)=data.CPMGT.signal(i)+ancillaSig(i);
            data.CPMGT.reference(i)=data.CPMGT.reference(i)+ancillaRef(i);
            data.CPMGT.roundsignal(i)=ancillaSig(i);
            data.CPMGT.roundreference(i)=ancillaRef(i);
            warning=0;
            i=i+1;
        end
    end
    disp('Saving Data after round')
    SaveDataRound('CPMGT', j, hObject, eventdata, handles) %Save data after each round
end
...
global Sent Seq samples len DC_off T1 data dataParameters DC Amp Detector awg buffer countsExport bGo sg
...
WriteSG(sg, 1,dataParameters.T1.freq, str2double(handles.MW_Power.String))
...
while i<=length(dataParameters.T1.vector) && bGo
    try
        ...
        AWG_Measure_T1(waiting,piPulse,handles);  %len.T1 defined here
        T1=GenAWGWF(T1,samples,len.T1);   %Convert to a form AWG accepts
        ...
        disp(strcat('Iteration', num2str(i), 'of', num2str(length(dataParameters.T1.vector))))
        for k=1:iteration_Repetitions %Repeat this iteration
            disp(strcat('Iteration repetition', num2str(k), 'of', num2str(iteration_Repetitions)))
            ...
            running=2;
            tic
            pause(loop_Standard*loop_Count*len.T1*10^-6+0.001)
            while running==2 && bGo
                fprintf(awg, '%s\n','AWGCONTROL:RSTATE?');
                running=str2double(fscanf(awg,'%s'));
                if isempty(running)
    ...
    if failed
        ...
        data.T1.signal(i)=data.T1.signal(i)+ancillaSig(i);
        data.T1.reference(i)=data.T1.reference(i)+ancillaRef(i);
        warning=0;
        i=i+1;
    end
end
...
if handles.(char('Infinite_Sequence')).Value
    ...
    for ii = 1:vecLen
        ...
        if detect
            ...
            %initialize data vectors
            ancillaSig(ii) = sigDatum;
            ancillaRef(ii) = refDatum;
...
dataParameters.AOM_Delay.min=950;
dataParameters.AOM_Delay.max=1150;
dataParameters.AOM_Delay.step=10;
dataParameters.AOM_Delay.vector= (dataParameters.AOM_Delay.min:dataParameters.AOM_Delay.step:dataParameters.AOM_Delay.max);
...
for SigTTL = dataParameters.AOM_Delay.vector
    ...
    CreateSeq(loop_Standard,loop_Count,1,'AOM_Delay'); %I changed the first number to 1000 from 200 but idk what I'm doing --Satcher
    %CreateInfiniteSeq('AOM_Delay',DC,Amp)
    % CreateSUBSeq(subLen,1,1,'Rabi');
    % CreateSeq_from_SUB(20,5000,1,'Rabi',DC,Amp);
    %Mark that Rabi is currently loaded on AWG
    ...
    strcat('Iteration', num2str(i), 'of', num2str(length(dataParameters.AOM_Delay.vector)))
...
Pi_Pulse_Calibrate.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    pulseTrainEnd+1101,pulseTrainEnd+1101+2800,1;... %Detection
    pulseTrainEnd+1101+2801,1000*len.Pi_Pulse_Calibrate,0];  %Keep AOM cool
...
Rabi.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    3001+MaxLength+1101,3001+MaxLength+1101+3000+800,1;... %Detection
    3001+MaxLength+1101+3801,1000*len.Rabi,0];  %Keep AOM cool
...
%%%%%% FPGA Reference %%%%%% 
Rabi.Mk4 = [3001+MaxLength+1101+3500,3001+MaxLength+1101+3500+300,1];
...
%%%%%% Laser %%%%%% 
ESR.Mk2 = [0,400,1;401,4000,0];  % 10% duty cycle
...
ODMR.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    3001+MWLength+1101,3001+MWLength+1101+3000+800,1;... %Detection
    3001+MWLength+1101+3801,1000*len.ODMR,0];  %Keep AOM cool
...
%%%%%% FPGA Reference %%%%%% 
ODMR.Mk4 = [3001+MWLength+1101+3500,3001+MWLength+1101+3500+300,1];
...
Ramsey.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    4153+piPulse+tauMax,4153+piPulse+tauMax+2000+800,1;... %Detection
    4153+piPulse+tauMax+2000+801,1000*len.Ramsey,0];  %Keep AOM cool
...
SpinEcho.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    4155+piPulse+2*tau+piPulse,4154+piPulse+2*tau+piPulse+3000+800,1;... %Detection
    4154+piPulse+2*tau+piPulse+3000+801,1000*len.SpinEcho,0];  %Keep AOM cool
...
%%%%%% FPGA Reference %%%%%% 
SpinEcho.Mk4 = [4154+piPulse+2*tau+piPulse+3500,4154+piPulse+2*tau+piPulse+3500+300,1];
...
%%%%%% Mixer control: I %%%%%% 
v_I=str2double(handles.Voltage_I.String);
%Start with +Pi/2 pulse and tau wait
iTemp = [4050,4050+piPulse/2,0,pi/2,v_I;4050+piPulse/2+1,4050+piPulse/2+1+tau,0,pi/2,0];
% Add Pi Pulses
iTemp = ItXY_Analog(iTemp, n, tau, piPulse,v_I,1); %ItXY_Analog(iTemp, n, tau, piPulse,v_I,1); 
%Add tau wait and -Pi/2 Pulse
iTemp(end+1,:)=[iTemp(end,2)+1,iTemp(end,2)+1+tau,0,pi/2,0];
...
len.CPMG = max(ceil((CPMG.Ch1(end,2)+51+2000+300+3000)/1000) , ceil((n+1)*piPulse/1000 * 10)); % Guarantee max 5% duty cycle for MW to avoid thermal heating. 10/8/16 by Chong
% len.CPMG=ceil((CPMG.Ch1(end,2)+51+2000+300+3000)/1000); %Add 3us after reference trigger

%%%%%% Mixer control: Q %%%%%% 
v_Q=str2double(handles.Voltage_Q.String);
iTemp = [4050,4050+piPulse/2,0,pi/2,0;4050+piPulse/2+1,4050+piPulse/2+1+tau,0,pi/2,0];
% Add Pi Pulses
iTemp = ItXY_Analog(iTemp, n, tau, piPulse,v_Q,0); 
...
if tau <=40
    ...
    %Add Pi Pulses
    iTemp = ItXY_Switch(iTemp, n, tau, piPulse, 1);
    %Add tau-100 wait
...
CPMG.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    CPMG.Ch1(end,2)+51,CPMG.Ch1(end,2)+51+2000+800,1;... %Detection
    CPMG.Ch1(end,2)+51+2000+801,1000*len.CPMG,0];  %Keep AOM cool
...
len.T1 = ceil((4072+2*piPulse+tau+201+2000+300+3000)/1000); %length of waveform in us

%%%%%% Mixer control: I Switch & Switch %%%%%% 
if handles.T1_m1.Value
...
%Switch is ~3ns before I and Q
%I and Q turn off 3ns before switch
%%%%%% Laser %%%%%% Markers must come in pairs
T1.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    4072+2*piPulse+tau+201,4072+2*piPulse+tau+201+2000+800,1; % Detection
    4072+2*piPulse+tau+201+2000+801,1000*len.T1,0];  %Keep AOM cool
...
function AWG_p_Duty_Cycle(handles)

global DC_p len Sent samples Seq

len.DC_p = 10 ; % length in us
p_DC=10%str2double(handles.p_DC.String); %get duty cycle percentage
p_DC=p_DC/100; %Normalize
if p_DC>1
    warning('Percentages higher than 100 are set to 100%')
    p_DC=1;
end

%%%%%% Mixer control: I %%%%%% 
DC_p.Ch1 = [0,0,0;0,1000*len.DC_p,0]; 

%%%%%% Switch %%%%%% 
DC_p.Mk1 = [0,0,0;0,1000*len.DC_p,0]; 

%%%%%% Laser %%%%%% 
if floor(p_DC*1000*len.DC_p)==1000*len.DC_p
    DC_p.Mk2 = [0,0,0;0,floor(p_DC*1000*len.DC_p),1]; 
else
    DC_p.Mk2 = [0,0,0;0,floor(p_DC*1000*len.DC_p),1;floor(p_DC*1000*len.DC_p)+1,1000*len.DC_p,0];
end


disp(' ')
disp(['--- Sending ' handles.p_DC.String '% Duty Cycle to AWG'])
DC_p=GenAWGWF(DC_p,samples,len.DC_p);
Plot_AWG_Sequence(DC_p,handles)
Sent.DC_p=SendWF2AWG(samples, len.DC_p, DC_p, struct,'DC_p');


disp(' ')
disp('--- Setting AWG sequence to DC_p')
CreateInfiniteSeq('DC_p')
Seq.DC_p=1;
Seq=Loop_Remaining_Structure({'DC_p'}, Seq,0);
RunAWG(1);
...
AWG2AOM = 810; % Measured 1160 on SPCM. -200ns for other factos
...
%Check that each channel is the same length
for str = ChnlCount
    if length(merged.(char(str)))~=length(merged.(char(ChnlCount{1})))
        disp(['Channel ' num2str(ChnlCount{1}) 'length not equal to Channel ' num2str(str) ' length'])
        error(['Channel ' num2str(ChnlCount{1}) 'length not equal to Channel ' num2str(str) ' length'])       
    end
end
...
function [signal, reference]=InitializeDAQSeq()
% Initialize DAQ
...
DAQmxFunctionPool('SetTwoGatedCounters',signal,reference,0); % Wait time of 0 hopefully means DAQ collects until the buffer is read out
end

function DAQmxStopClearTask(task)
    DAQmxStopTask(task);
    DAQmxClearTask(task);    
end
...
global gSaveData data dataParameters DC Amp gScan
...
fprintf(fid, 'NV Position: Vx = %.4f Vy = %.4f Vz = %.4f\n',[gScan.FixVx gScan.FixVy gScan.FixVz]);
...
fprintf(fid,'\nNotes: %s',nts);
fprintf(fid,'\nTime: %s',datestr(now, 'yyyy-mm-dd_HH-MM-SS'));
...
handles.Data_File_Num.String = num2str(ImgN);

end

function SaveDataRound(name, roundNum, hObject, eventdata, handles)
%name should be a string
...
ImgN = ImgN + 1;
file = strrep(file,'.txt',sprintf(['_Round_' num2str(roundNum) '_Data%03d.txt'],ImgN));
%Save File as Data
Sig= data.(char(name)).roundsignal;
Ref= data.(char(name)).roundreference;
%CurrentVz = DAQmxFunctionPool('ReadVoltage',);
fid = fopen(file,'wt');
...
global gSavePlots data dataParameters fx fxrange fitFnx fit
...
%order.
% date = [num2str(now(1)),'-',num2str(now(2)),'-',num2str(round(now(3)))];
file = [gSavePlots.path strcat(name, gSavePlots.file.SigRef)];
% file = [gSaveData.path strcat(name, ['_Sig_Ref_Plot_' date '.fig'])];
mfile = strrep(file,'.fig','*');
mfilename = strrep(gSavePlots.file.SigRef,'.fig','');
...
%Save second plot
file = [gSavePlots.path strcat(name, gSavePlots.file.SigOverRef)];
mfile = strrep(file,'.fig','*');
mfilename = strrep(gSavePlots.file.SigOverRef,'.fig','');
...
if strcmp(char(name),'ESR')
    plot(single(Indep),single(Sig),'-bo') 
else
...
if firstQ
    ...
    %Create next command in waveform
    aTemp(1)=aTemp(2)+1; %Next time is 1ns after last time
    aTemp(2)=aTemp(1)+tau-40; %wait tau-100ns
    aTemp(3)=0; %wait tau
    %Add command to waveform
    a(sz+2*i-1,:)=aTemp;
...
function a=ItXY_Analog(b,n,tau,PiPulse,amp,XorY)

a=b; %leave original array unchanged
sz=size(a);
sz=sz(1);
aTemp=a(end,:); %Extract last row of a
a=[a;zeros(2*n-1,5)]; %preallocate waveform commands

iStart = 1;
iEnd = n;

if n==0 %If n=0, mod is tricked.
...
for i=iStart:iEnd-1
    ...
    % Create Pi pulse
    aTemp = XY8_Pulses(aTemp,i,amp,PiPulse,XorY);
    a(sz+2*i-1,:) = aTemp ;
...
aTemp = XY8_Pulses(aTemp,iEnd,amp,PiPulse,XorY);
a(sz+2*iEnd-1,:) = aTemp ;


end

function Pulse=XY8_Pulses(a,n,amp,PiPulse,XorY)

nX=[0,1,3,6];
aTemp=a(end,:); %Extract last row of a

if ~isempty(find(nX==mod(n,8),1)) %Then current pulse is on X
    if XorY %We are creating X pulses
        aTemp(1)=aTemp(2)+1; %Next time is 1ns after last time
        aTemp(2)=aTemp(1)+PiPulse; %Pi pulse
        aTemp(3) = 0 ;
        aTemp(4)=pi/2;
...
else %Then current pulse is on Y
    if XorY %We are creating X pulses
        aTemp(1)=aTemp(2)+1; %Next time is 1ns after last time
        aTemp(2)=aTemp(1)+PiPulse; %Pi pulse
        aTemp(3) = 0 ;
...
fx = lsqcurvefit(lorB,[-1*(max(sig)-min(sig)),xrange(round(length(xrange)/2)),0.02,max(sig)],xrange,sig)
...
sig=data.Pi_Pulse_Calibrate.signal./data.Pi_Pulse_Calibrate.reference;
...
function Stop_After_Round(handles)
global bGoR;
...
global dataTempSig dataTempRef data data1Sig data1Ref data2Sig data2Ref

% %Initialize
% dataTempSig=zeros(1,201);
% dataTempRef=zeros(1,201);
% i=1;
% failed=false;
% while i<20
%     disp(['i = ', num2str(i)])
...
%     else
%         i=i+1;
%         dataTempSig=dataTempSig+data.SpinEcho.signal;
%         dataTempRef=dataTempRef+data.SpinEcho.reference;
%         failed=false;
%     end
% end
% 
% data1Sig=dataTempSig;
% data1Ref=dataTempRef;

%Initialize
handles.T1_m1.Value=1;
handles.T1_0.Value=0;
dataTempSig=zeros(1,21);
dataTempRef=zeros(1,21);
i=1;
failed=false;
while i<20
    disp(['i = ', num2str(i)])
    ...
    if failed
        ...
        i=i+1;
        dataTempSig=dataTempSig+data.T1.signal;
        dataTempRef=dataTempRef+data.T1.reference;
        failed=false;
    end
end

data2Sig=dataTempSig;
data2Ref=dataTempRef;

...
%%%%Split Rabi waveform into 1000 point sub-waveforms and send each
Plot_AWG_Sequence(Rabi,handles)
subLen=10; %Number of times to divide waveform
lR=length(Rabi.Ch1);
present=struct; %Blank structure so SendWF2AWG sends on all iterations of loop
for kk=0:subLen-1
    
    RabiSub.Ch1=Rabi.Ch1((kk*lR/subLen+1):(kk+1)*lR/subLen);
    RabiSub.Ch2=Rabi.Ch2((kk*lR/subLen+1):(kk+1)*lR/subLen);
    RabiSub.Ch3=Rabi.Ch3((kk*lR/subLen+1):(kk+1)*lR/subLen);
    RabiSub.Mk1=Rabi.Mk1((kk*lR/subLen+1):(kk+1)*lR/subLen);
    RabiSub.Mk2=Rabi.Mk2((kk*lR/subLen+1):(kk+1)*lR/subLen);
    RabiSub.Mk3=Rabi.Mk3((kk*lR/subLen+1):(kk+1)*lR/subLen);
    RabiSub.Mk4=Rabi.Mk4((kk*lR/subLen+1):(kk+1)*lR/subLen);
    RabiSub.Mk5=Rabi.Mk5((kk*lR/subLen+1):(kk+1)*lR/subLen);
    RabiSub.Mk6=Rabi.Mk6((kk*lR/subLen+1):(kk+1)*lR/subLen);
    SentRabi=SendWF2AWG(samples, len.rabi, RabiSub, present,['Rabi_' num2str(kk+1) '_']);
end
...
ancillaSig(1) = data(1)*65536 + data(2)*256+data(3); %Convert Signal data
ancillaRef(1) = data(4)*65536 + data(5)*256+data(6); %Convert Reference data
% 
% i=2;
% %On Remaining runs, we have to load new MW elements
% for MWLength == 5:5:1000
%     disp(' ')
%     disp('--- Sending Rabi to AWG')
%     AWG_Measure_Rabi(MWLength);
...
%pause(1) %
% data = fread(Detector,6); %Read data from FPGA
% ancillaSig(i) = data(1)*65536 + data(2)*256+data(3); %Convert Signal data
% ancillaRef(i) = data(4)*65536 + data(5)*256+data(6); %Convert Reference data
% i=i+1;    
%     
% end
...
if ~isempty(sn) %Is PreSent is a structure with no fieldnames
    for n=1:length(sn)
        ...
        if strcmp(sn{n},'Ch1') %If it's Channel 1
            'ch1'
            plot(handles.axes1, single(struct.Ch1)/norm(single((struct.Ch1))),'-r')
            hold on
            'mk1'
            plot(handles.axes1,single(struct.Mk1)/norm(single(struct.Mk1))+1.1, '-r','LineStyle','--')
            plot(handles.axes1,single(struct.Mk2)/norm(single(struct.Mk2))+2.2, '-r','LineStyle','-.')
        elseif strcmp(sn{n},'Ch2') %If it's Channel 2
            plot(handles.axes1,single(struct.Ch2)/norm(single(struct.Ch2))+3.3,'-b')
            hold on
            plot(handles.axes1,single(struct.Mk3)/norm(single(struct.Mk3))+4.4, '-b','LineStyle','--')
            plot(handles.axes1,single(struct.Mk4)/norm(single(struct.Mk4))+5.5, '-b','LineStyle','-.')
        elseif strcmp(sn{n},'Ch3') %If it's Channel 3
            plot(handles.axes1,single(struct.Ch3)/norm(single(struct.Ch3))+6.6,'-g')
            hold on
            plot(handles.axes1,single(struct.Mk5)/norm(single(struct.Mk5))+7.7, '-g','LineStyle','--')
            plot(handles.axes1,single(struct.Mk6)/norm(single(struct.Mk6))+8.8, '-g','LineStyle','-.')
        elseif strcmp(sn{n},'Ch3') %If it's Channel 4
            plot(handles.axes1,single(struct.Ch4)/norm(single(struct.Ch4))+9.9,'-k')
            hold on
            plot(handles.axes1,single(struct.Mk7)/norm(single(struct.Mk7))+11, '-k','LineStyle','--')
            plot(handles.axes1,single(struct.Mk8)/norm(single(struct.Mk8))+12.1, '-k','LineStyle','-.')
        end

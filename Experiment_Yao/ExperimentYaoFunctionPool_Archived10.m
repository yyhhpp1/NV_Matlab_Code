...
global SentRabi SentDeer SentUser Seq samples len Readout DC_off gSaveData data Detector awg buffer
...
global SentRabi Seq samples len DC_off Rabi data dataParameters DC Amp Detector awg buffer
...
global SentESR Seq samples len DC_off ESR data dataParameters Rabi DC Amp Detector awg buffer
...
DC=[DC_off.I, DC_off.Q ,0,2.25]; %Offsets of Ch1 2 3 4
Amp= [1,1,3.3,3,2.2]; %[dataParameters.ESR.power,dataParameters.ESR.power,0.05,2,2.2]; %Amp of Ch1 2 3 4, and Markers in V
%Smallest permitted amplitude is 50mV
...
AWG_Measure_ESR(4000); %MWLength Set to Pi Pulse
...
for freq = dataParameters.ESR.vector
...
global User Seq samples len DC_off data dataParameters DC Amp Detector awg buffer
...
if handles.(char('Infinite_Sequence')).Value
    ...
    for str=pulses
        ...
        mod = dataParameters.User.(char(str)).string;
        if strcmp(mod,'Amp')
            User_DC_AMP(dataParameters.User.(char(str)).Offset, dataParameters.User.(char(str)).vector(i),str);
        end
    end
    CreateInfiniteSeq('User',DC,Amp);
    OutputAWG(1,1,1,1,0); %Run Sequence (FPGA collects)
    disp(' ')
    ...
    for ii = 1:vecLen
        ...
        CreateSeq(20,50000,1,'User',DC,Amp); %20*50000=10^6 measurements
...
global SentAOM Seq samples len DC_off AOM_Delay data dataParameters Detector awg buffer
...
global samples buffer ChnlCount awg
...
function writeSG(serial, freq, dBm, on)
% This function sets the output of the SignalCore SC5511A to a given
% frequency and power. serial is the unique char-type identifier of your
% device ('10000EDA' is the one that the Yao lab owns as of 06/2016). freq
% is the frequency in Hz. dBm is the power in dBm. on is a logical which
% determines whether to turn on the output.
...
p=libpointer('uint32Ptr',0);
calllib('sg','Sc5511a_open_device',serial,p);
calllib('sg','Sc5511a_set_rf_mode',0,p.Value,libpointer);
calllib('sg','Sc5511a_set_level',dBm,p.Value,libpointer);
calllib('sg','Sc5511a_set_freq',freq,p.Value,libpointer);
calllib('sg','Sc5511a_set_output',on,p.Value,libpointer);
calllib('sg','Sc5511a_close_device',p.Value);
end
...
% function ESR(hObject, eventdata, handles)
% 
% global SentESR Seq samples len DC_off ESR data dataParameters Rabi DC Amp
% 
% 
% % 
% 
...
% dataParameters.ESR.vector=(dataParameters.ESR.min:dataParameters.ESR.step:dataParameters.ESR.max);
% dataParameters.ESR.power=str2double(handles.ESR_Power.String) ;
% len.ESR = 3 ; %length of waveform in us
% 
% DC=[DC_off.I, DC_off.Q ,0,2.25]; %Offsets of Ch1 2 3 4
% Amp= [1,1,3.3.05,2,2.2]; %[dataParameters.ESR.power,dataParameters.ESR.power,0.05,2,2.2]; %Amp of Ch1 2 3 4, and Markers in V
% %Smallest permitted amplitude is 50mV
% 
% % %%%Begin Infinite Sequence
% % MWFrequency=1000; %ns
...
% disp('--- Sending ESR to AWG')
% %ESR Sequence is simply Rabi sequence with a fixed MW Length
% AWG_Measure_ESR(3000); %MWLength Set to Pi Pulse
% ESR_Handshake;

...
% Populate Parameters
fullPath = 'C:\MATLAB Scripts\GUI\Experiment_Yao\';
...
while i<=length(dataParameters.Rabi.vector) && bGo
    try
        ...
        for k=1:iteration_Repetitions %Repeat this iteration
            ...
            %initialize data vectors
            ancillaSig(i) = ancillaSig(i) + dataTemp(1)*65536 + dataTemp(2)*256+dataTemp(3); %Convert Signal data
            ancillaRef(i) = ancillaRef(i) + dataTemp(4)*65536 + dataTemp(5)*256+dataTemp(6); %Convert Reference data
        end
...
global Sent Seq samples len DC_off ESR data dataParameters DC Amp Detector awg buffer countsExport bGo bGoR
...
while i<=length(dataParameters.ODMR.vector) && bGo
    try
        ...
        if ancillaRef(i)>TheoreticalThreshold
            ...
            disp('Reference counts too low. Try again.')
            AWG_p_Duty_Cycle(handles) %Need to switch back to a usable sequence
            TrackNV(hObject,eventdata);
            while countsExport<0.9*Count
                TrackNV(hObject,eventdata);
...
global Sent Seq samples len DC_off Ramsey data dataParameters DC Amp Detector awg buffer countsExport bGo bGoR yn
...
%user.
if loop_Standard*loop_Count*iteration_Repetitions*dataParameters.Ramsey.rounds>10^5
   tooManyIterations
   waitfor(1010)
   if ~yn
       'Abort measurement'
...
global Sent Seq samples len DC_off SpinEcho data dataParameters DC Amp Detector awg buffer countsExport bGo bGoR yn
...
TrackNV(hObject, eventdata)
Count = countsExport; %1.08*10^5; %Counts per second Update from ImageNVC
TheoreticalThreshold = 0.7*(300*10^-9)*loop_Standard*loop_Count*iteration_Repetitions*Count %A reference data point should be around 30k counts for 10^6 measurements
    ...
    while i<=length(dataParameters.SpinEcho.vector) && bGo
        ...
        try
            ...
            if ancillaRef(i)>TheoreticalThreshold
                ...
                warning('Reference counts too low. Try again.')
                error('Reference Counts Low')
            end
        catch
            failed=true;
...
Rabi.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    3001+MaxLength+1101,3001+MaxLength+1101+2000+800,1;... %Detection
    3001+MaxLength+1101+2801,1000*len.Rabi,0];  %Keep AOM cool
...
ODMR.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    3001+MWLength+1101,3001+MWLength+1101+2800,1;... %Detection
    3001+MWLength+1101+2801,1000*len.ODMR,0];  %Keep AOM cool
...
SpinEcho.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    4155+piPulse+2*tau+piPulse,4154+piPulse+2*tau+piPulse+2000+800,1;... %Detection
    4154+piPulse+2*tau+piPulse+2000+801,1000*len.SpinEcho,0];  %Keep AOM cool
...
AWG2AOM = 960; % Measured 1160 on SPCM. -200ns for other factos

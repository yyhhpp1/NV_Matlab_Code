...
for ichn=1:numel(SEQ.CHN)
    for irise = 1:SEQ.CHN(ichn).NRise
        ...
        EventM(event,3) = SEQ.CHN(ichn).T(irise) + ...
            SEQ.CHN(ichn).DT(irise) - SEQ.CHN(ichn).Delays(2);
        %EventM(event,4) = 0;
        event = event +1;
    end
end
...
EventM(:,2) = 1e9*EventM(:,2);
EventM(:,3) = 1e9*EventM(:,3);
...
for pb=1:NPB-1
    PBEvents(pb,2) = PBEvents(pb+1,2)-PBEvents(pb,2);
end
PB = PBEvents(1:NPB-1,:);
NPB = NPB - 1;
...
if strcmp(CMD(1,2),CONTINUE)
    ...
    AuxCMD(a,:) = [CMD{1,1},LOOP,SEQ.Samples,CMD{1,4},CMD{1,5}];
elseif strcmp(CMD(1,2),LONG_DELAY)
    AuxCMD(a,:) = [CMD{1,1},LOOP,SEQ.Samples,MinDelay,CMD{1,5}];
    a = a +1;
    aux= CMD{1,3};

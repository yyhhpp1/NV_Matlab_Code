...
for kk=floor(elem_ind):loop_standard
    ...
    for str = ChnlCount
        if strcmp(str,'Ch1')
            fprintf(awg, '%s\n',['SEQ:ELEM' num2str(kk) ':WAV1 "' name 'Ch1"']);
        elseif strcmp(str,'Ch2')
            fprintf(awg, '%s\n',['SEQ:ELEM' num2str(kk) ':WAV2 "' name 'Ch2"']);
        elseif strcmp(str,'Ch3')
            fprintf(awg, '%s\n',['SEQ:ELEM' num2str(kk) ':WAV3 "' name 'Ch3"']);
        elseif strcmp(str,'Ch4')
            fprintf(awg, '%s\n',['SEQ:ELEM' num2str(kk) ':WAV4 "' name 'Ch4"']);
            %fprintf(awg,'%s\n','SEQ:ELEM1:WAV1 "AOM_DelayCh1";*esr?;WAV2 "AOM_DelayCh2";*esr?;WAV3 "AOM_DelayCh3";*esr?;*esr?;LOOP:COUN 5000;*esr?')
            %fscanf(awg,'%s')
        else
            fclose(awg);
            delete(awg);    %delete instrument object
    ...
    %Number of times to loop element is max 65536 if you don't hit the
    %waveform memory limit first
    fprintf(awg,'%s\n','*ESR?;SYST:ERR?'); %Query so Matlab and AWG stay synchronized
    fscanf(awg,'%s');
end

...
while i<=length(dataParameters.Rabi.vector) && bGo
    try
        ...
        for k=1:iteration_Repetitions %Repeat this iteration
            ...
            running=2;
            tic
            pause(loop_Standard*loop_Count*len.Rabi*10^-6+0.001)
            while running==2 && bGo
                fprintf(awg, '%s\n','AWGCONTROL:RSTATE?');
...
fprintf(awg, '%s\n',['SEQ:LENG ' num2str(0)]); % Remove ESR from AWG sequence so we don't accidentally apply high power MW indefinitely
...
%%%%%% Mixer control: Q %%%%%% 
v_Q=str2double(handles.Voltage_Q.String);
iTemp = [4050,4050+piPulse/2+tau+piPulse, 0,0,0];
iTemp = ItXY_Analog(iTemp, n/2, tau, piPulse,v_Q,0,0); %Originally ItXY(iTemp, n/2, tau, piPulse);
iTemp(end,2) = iTemp(end,2)-piPulse ; %ItXY automatically adds undesired wait for PiPulse at end
iTemp(end+1:end+2,:)=[iTemp(end,2)+1,iTemp(end,2)+1+piPulse, 0,pi/2,v_Q; iTemp(end,2)+2+piPulse,iTemp(end,2)+2+2*piPulse+2*tau,0,0,0];
...
CPMG.Mk2 = [0,3000,1;... %Polarize for 3us
    ...
    CPMG.Ch1(end,2)+51,1000*len.CPMG,1];  %Detection and keep AOM warm
...
function PiPulse=XY8_Pulses(a,n,XorY)

nX=[0,1,3,6];
aTemp=a(end,:); %Extract last row of a

if ~isempty(find(nX==mod(n),1)) %Then current pulse is on X

...
fopen(awg);
fprintf(awg, '*CLS');
fprintf(awg,'*ESR?'); %Query to ensure Matlab wait sufficiently
fscanf(awg,'%s');
fprintf(awg, 'SOUR1:ROSC:SOUR EXTERNAL;FREQ 100MHZ;*ESR?');
fscanf(awg,'%s');
...
len.Pi_Pulse_Calibrate = ceil((4050+number*(MWLength+20)+1101+2000+300+500)*10^-3); %length of waveform in us
...
for j = 1:rounds
    ...
    while i<=length(dataParameters.SpinEcho.vector) && bGo
        ...
        if data.SpinEcho.reference(i)>TheoreticalThreshold
            i=i+1;
            warning=0;
        else
            TrackNV(hObject,eventdata);
...
fprintf(awg,'*ESR?;SYST:ERR?'); %Query to ensure Matlab wait sufficiently
fscanf(awg,'%s');
seqNo=0;
fprintf(awg,['WLIST:WAVEFORM:LENGTH? "' name (fnB{1}) '"']); %find length of waveform on AWG if it's there
...
for n = 1:length(fnB)
    if Sent.(char(name))==0 || isempty(lenPre) || lenPre ~=double(len*samples)
        ...
        fprintf(awg,['WLIST:WAV:DEL "' name (fnB{n}) '";:WLIST:WAV:NEW "' name (fnB{n}) '",' num2str(len*samples) ',real;'])
...
if buffer >= bytes  % If buffer can handle it, proceed
    ...
    for n=1:length(cmd) %This loop must be out here when EOI off
        ...
        for i = 1:buffer:bytes-buffer
            ...
            fprintf(awg,'%s',cmd{n}(i:i+buffer-1))
        end
        %awg.Terminator = 'LF';
        i=i+buffer;
        %length(cmd(i:end))
        fprintf(awg,cmd{n}(i:end));
        fprintf(awg,'*ESR?'); %Query to ensure Matlab wait sufficiently
        fscanf(awg,'%s');
    end
end
...
% err=0;
% for n = 1:length(fnB)
%     fprintf(awg,[':wlist:waveform:data? "' name (fnB{n}) '"']); 
%     ch=fscanf(awg,'%s');
%     
%     if isempty(ch)
...
%%%%%% DC offset %%%%%% 
fprintf(awg,['sour1:volt:offs ' num2str(DC.Ch1)])
fprintf(awg,['sour2:volt:offs ' num2str(DC.Ch2)])
fprintf(awg,['sour3:volt:offs ' num2str(DC.Ch3)])
fprintf(awg,['sour4:volt:offs ' num2str(DC.Ch4)])
fprintf(awg,'*ESR?;SYST:ERR?'); %Query to ensure Matlab wait sufficiently
fscanf(awg,'%s');

%%%%%% amp %%%%%% 
fprintf(awg,['sour1:volt:ampl ' num2str(Amp.Ch1)])
fprintf(awg,['sour2:volt:ampl ' num2str(Amp.Ch2)])
fprintf(awg,['sour3:volt:ampl ' num2str(Amp.Ch3)])
fprintf(awg,['sour4:volt:ampl ' num2str(Amp.Ch4)])
fprintf(awg,'*ESR?;SYST:ERR?'); %Query to ensure Matlab wait sufficiently
fscanf(awg,'%s');
fprintf(awg,['sour1:mark1:volt:low ' num2str(DC.Mk1)])
fprintf(awg,['sour1:mark2:volt:low ' num2str(DC.Mk2)])
fprintf(awg,['sour2:mark1:volt:low ' num2str(DC.Mk3)])
fprintf(awg,['sour2:mark2:volt:low ' num2str(DC.Mk4)])
fprintf(awg,['sour3:mark1:volt:low ' num2str(DC.Mk5)])
fprintf(awg,['sour3:mark2:volt:low ' num2str(DC.Mk6)])
fprintf(awg,['sour4:mark1:volt:low ' num2str(DC.Mk7)])
fprintf(awg,['sour4:mark2:volt:low ' num2str(DC.Mk8)])
fprintf(awg,'*ESR?;SYST:ERR?'); %Query to ensure Matlab wait sufficiently
fscanf(awg,'%s');
fprintf(awg,['sour1:mark1:volt:high ' num2str(Amp.Mk1)])
fprintf(awg,['sour1:mark2:volt:high ' num2str(Amp.Mk2)])
fprintf(awg,['sour2:mark1:volt:high ' num2str(Amp.Mk3)])
fprintf(awg,['sour2:mark2:volt:high ' num2str(Amp.Mk4)])
fprintf(awg,['sour3:mark1:volt:high ' num2str(Amp.Mk5)])
fprintf(awg,['sour3:mark2:volt:high ' num2str(Amp.Mk6)])
fprintf(awg,['sour4:mark1:volt:high ' num2str(Amp.Mk7)])
fprintf(awg,['sour4:mark2:volt:high ' num2str(Amp.Mk8)])
fprintf(awg,'*ESR?;SYST:ERR?'); %Query to ensure Matlab wait sufficiently
fscanf(awg,'%s');
...
for kk=floor(elem_ind):loop_standard
    ...
    fprintf(awg,strcat(seqCmd,['LOOP:COUN ' num2str(loop_count) ';*ESR?;:SYST:ERR?']));
    fscanf(awg,'%s');
    %Number of times to loop element is max 65536 if you don't hit the
    %waveform memory limit first
end

%fprintf(awg,['SEQ:ELEM' num2str(loop_standard) ':GOTO:STAT 1']) %%Turns on the GoTo:Index ability for the last element in the sequence
%fprintf(awg,['SEQ:ELEM' num2str(loop_standard) ':GOTO:IND 1']) %%Goes to the first index (index 1) in the last element of the sequence

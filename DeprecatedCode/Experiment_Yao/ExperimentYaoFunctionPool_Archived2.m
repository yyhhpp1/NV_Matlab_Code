...
disp(' ')
disp('--- Setting AWG sequence to Rabi')
CreateSeq(20,5000,1,Rabi,'Rabi',DC,Amp);
%Mark that Rabi is currently loaded on AWG
Seq.Rabi = 1 % Redundant with the following function
Loop_Remaining_Structure('Rabi', Seq);
% 
...
if isstruct(PreSent)
    ...
    if isempty(sn) %Is PreSent is a structure with no fieldnames
        ...
        for str = sn'
            if PreSent.(char(str)) == 0 %If is wasn't sent...
                struct.(char(str)) = structure.(char(str));
    ...
    if  strcmp(fn{n}(1:2),'Ch') %Analog 
        ...
        if mod(str2num(fn{n}(3)),2) %If Marker is odd
            ...
            elseif strcmp(fn{n}(3),'5') %If it's Marker 5
                ...
                fn{n}
                merged.Ch4=struct.Mk5+struct.Mk6 %check dec2bin(m(2),8)
            end
        end
    end
end
...
%     end
%     
%     if length(struct.(fn{n}))==5 %Analog 
%         'Analog'
%         fn{n}
%         struct.(fn{n}) = Wave_generator(struct.(fn{n}),samples,len);
%         skip=0;
%         ChnlCount=ChnlCount+1; % Since all digital outputs must also have 
%         %an analog output, counting analog outputs is equivalent to
%         %counting the number of channels used.
%     else %Digital
%         'Digital'
...
%         
%         %%%% merge markers %%%%
%         merged(end+1)=struct.(fn{n})+struct.(fn{n+1}) %check dec2bin(m(2),8)
%         
%         %Already processed next element of struct
%         skip=1;
%     end
% end
...
cmd=cell(length(fnB)) %Create cell to hold commands

for n = 1:length(fnB)
    fwrite(awg,['wlis:wav:del "' name (fnB{n}) '"'])
    %Create an empty waveform with name and datasize
    fwrite(awg,[':wlist:waveform:new "' name (fnB{n}) '",' num2str(samples) ',real;']);

...
cfg.plotting.rules = { ...
    ...
    'T1_Sij_all', 'PlotT1Data_9curves', ''; ...
    'Echo', 'plotting.plot_echo_data', ''; ...
    'Ramsey', 'plotting.plot_echo_data', '' ...
};

cfg.smart = struct();
cfg.smart.enabled = true;
cfg.smart.measurements = default_measurement_cfg();
cfg.smart.sijAll = struct();
cfg.smart.sijAll.enabled = false;
cfg.smart.sijAll.sequenceName = 'T1_Sij_all';
cfg.smart.sijAll.firstIntervalNPoints = 8;
cfg.smart.sijAll.secondIntervalNPoints = 6;
cfg.smart.sijAll.thirdIntervalMinNPoints = 4;

cfg.smart.physics = struct();
cfg.smart.physics.zeroFieldGHz = 2.877;
cfg.smart.physics.gammaMHzPerG = 2.8025;

% Microwave delivery frequency range.
cfg.smart.mwDelivery = struct();
cfg.smart.mwDelivery.minGHz = 0.7;
cfg.smart.mwDelivery.maxGHz = 6.0;
...
cfg.smart.rough.maxStopNs = 2e9+1000; % hard cap for rough/final stop (ns). inf for no cap
...
% T2/T2* stretched fit selector.
% Model: A * exp(-(x/t)^n)
cfg.smart.t2fit = struct();
cfg.smart.t2fit.nLower = 0.5;
cfg.smart.t2fit.nUpper = 3.0;
cfg.smart.t2fit.amplitudeUpper = 1.0;
...
cfg.smart.precal.calipi.memoryTargetPiToleranceNs = 15; % only use PiCal memory rows with |pi_memory-targetPi| <= tol
...
% Global inputs.
t.measure.t1 = 'chk_enable_t1_family';
t.measure.t2 = 'chk_enable_t2_family';
t.measure.t2star = 'chk_enable_t2star_family';
t.measure.t1SijAll = 'chk_t1_sij_all_override';
t.input.estimatedB = 'edit_estimated_B_G';
t.input.nonuniform = 'chk_smart_point_distribution';
t.input.addLateTimePoints = 'chk_add_late_time_points';
...
% Final T1 auto-stop panel fields.
t.finalT1.enable = 'chk_final_t1_auto_stop_relerr';
t.finalT1.relErr = 'edit_final_t1_relerr';
t.finalT1.minAverage = 'edit_final_t1_min_average';
...
t.display.runElapsed = 'txt_run_elapsed';
...
% Per-target T2 fields.
t.t2.aligned_sq_m1 = target_family_tags('aligned_sq_m1', 't2');
t.t2.aligned_sq_p1 = target_family_tags('aligned_sq_p1', 't2');
t.t2.aligned_dq = target_family_tags('aligned_dq', 't2');
t.t2.off_sq_m1 = target_family_tags('off_sq_m1', 't2');
t.t2.off_sq_p1 = target_family_tags('off_sq_p1', 't2');
t.t2.off_dq = target_family_tags('off_dq', 't2');

% Per-target T2* fields.
t.t2star.aligned_sq_m1 = target_family_tags('aligned_sq_m1', 't2star');
t.t2star.aligned_sq_p1 = target_family_tags('aligned_sq_p1', 't2star');
t.t2star.aligned_dq = target_family_tags('aligned_dq', 't2star');
t.t2star.off_sq_m1 = target_family_tags('off_sq_m1', 't2star');
t.t2star.off_sq_p1 = target_family_tags('off_sq_p1', 't2star');
t.t2star.off_dq = target_family_tags('off_dq', 't2star');
...
function out = target_family_tags(prefix, family)
out = struct();
base = ['edit_' prefix '_' family];
out.start = [base '_start'];
out.stop = [base '_stop'];
out.nPoints = [base '_npts'];
out.repeat = [base '_repeat'];
out.average = [base '_average'];
end
...
function m = default_measurement_cfg()
m = struct();
m.t1 = struct( ...
    'enabled', true, ...
    'displayName', 'T1', ...
    'roughStopEstimator', 'single_exp', ...
    'analysisFamily', 't1', ...
    'supportedTargetIds', {{'aligned_sq_m1', 'aligned_sq_p1', 'aligned_dq', 'off_sq_m1', 'off_sq_p1', 'off_dq'}}, ...
    'sequenceByTransition', struct( ...
        'SQ_0_TO_M1', 'T1_S00_S01_S10', ...
        'SQ_0_TO_P1', 'T1_S00_S01_S10', ...
        'DQ_M1_TO_P1', 'T1_S11_S1m1'));
m.t2 = struct( ...
    'enabled', false, ...
    'displayName', 'T2', ...
    'roughStopEstimator', 'single_exp', ...
    'roughFitFamily', 't2_stretched', ...
    'analysisFamily', 't2', ...
    'supportedTargetIds', {{'aligned_sq_m1', 'aligned_sq_p1', 'off_sq_m1', 'off_sq_p1'}}, ...
    'sequenceByTransition', struct( ...
        'SQ_0_TO_M1', 'Echo', ...
        'SQ_0_TO_P1', 'Echo'));
m.t2star = struct( ...
    'enabled', false, ...
    'displayName', 'T2*', ...
    'roughStopEstimator', 'edge_ratio', ...
    'roughFitFamily', 't2_stretched', ...
    'analysisFamily', 't2star', ...
    'supportedTargetIds', {{'aligned_sq_m1', 'aligned_sq_p1', 'off_sq_m1', 'off_sq_p1'}}, ...
    'sequenceByTransition', struct( ...
        'SQ_0_TO_M1', 'Ramsey', ...
        'SQ_0_TO_P1', 'Ramsey'));

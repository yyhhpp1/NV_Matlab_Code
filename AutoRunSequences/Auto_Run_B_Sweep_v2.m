function Auto_Run_B_Sweep_v2(hObject, eventdata, handles)
    ...
    freq = 1944.72;
    Sweep_lst = [1, 2, 4, 8];
    Pi_lst = [200, 108, 60, 38];
    TO1_lst = [100100, 100100, 300100, 800100]; 


    %------------------ MAIN LOOP -----------------------------------------
    for i = 1:length(Sweep_lst)
        FPGA_Gain = Sweep_lst(i);
        Pi_time = Pi_lst(i);
        TO1 = TO1_lst(i);
        ...
        try
            % Create folder
            savePath = ensureFolder(SAVE_ROOT, FPGA_Gain);
            ...
            set(handles.pi,'String',string(Pi_time));
            set(handles.FPGAFreq7,'String',string(freq));
            set(handles.FPGAGain7,'String',string(FPGA_Gain));
            setGUIParams(handles, struct('FROM1',0,'TO1',TO1,...
                'SweepNPoints',21,'Repeat',1000,'Average',5));
            Auto_LoadUserInputs(hObject,eventdata,handles);
            RunSequence(hObject,eventdata,handles);
            safeScreenshot(handles.figure1, savePath);
            if ~gmSEQ.bAutoRun, return; end

        catch ME
            warning('Error at Gain = %.1f: %s', FPGA_Gain, ME.message);
        end
    end
    ...
    function savePath = ensureFolder(root, B)
        savePath = root + sprintf("Gain_%d_G", B);
    ...
    function pitime = runRabi(f, g, p_guess, path)
        gmSEQ.name = "f_Rabi";
        setSequence(handles, "f_Rabi");
        set(handles.FPGAGain7,'String',string(g));
        set(handles.FPGAFreq7, 'String', string(f*1e3));

        rabi_range = [10, p_guess*2+10];
        setGUIParams(handles, struct( ...
            'FROM1', rabi_range(1), 'TO1', rabi_range(2), ...
            'SweepNPoints', 21, 'misc', 1, 'Repeat', 1000, 'Average', 2));
        ...
        opts = optimoptions('lsqcurvefit','Display','off');
        popt = lsqcurvefit(sineModel,p0,x,y,lb,ub,opts);
        pitime = round((pi - popt(3))/pi * popt(2));
    ...
    function runSQT1(f, gain, p, path)
        ...
        set(handles.FPGAFreq7,'String',string(f*1e3));
        set(handles.FPGAGain7,'String',string(gain));
        setGUIParams(handles, struct('FROM1',1000,'TO1',4001000,...

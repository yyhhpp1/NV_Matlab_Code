% TODO: turn on a continous drive during T1 dark time
% have not finished
...
T=[t_R+i+d+p3+post_p3, u+m, u+i+d, u+i+t_R+i+d+p3+post_p3, u+m+p2+u+i+d];
DT=[p];
DT = repmat(DT, 1, 5);
ConstructSeq(T,DT)
...
T=[t_R+i+d, post_p3+p+u+m+p2+u+i*3+t_R*2+d];
DT=[p3, p3];
ConstructSeq(T,DT)

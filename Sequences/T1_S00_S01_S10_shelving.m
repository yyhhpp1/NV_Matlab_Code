function T1_S00_S01_S10_shelving
global gmSEQ gSG
gSG.bfixedPow=1;
gSG.bfixedFreq=1;
gSG.bMod='LOL';
...
t_T1 = d+p+u+m+p+u+p2+post_p2+p2+post_p2; %time betweeen two laser pulse for measurements
...
T=[...
   t_R+i+t_T1+i+d,                    ...     S00
   u+i+t_R+i+t_T1-u-p2-post_p2-p, post_p2+p2+u+i+d,          ...     S01
   u+i+t_R+i+d+p2+post_p2, u+m+p+post_p2+p2+u+i+d          ...     S10
   ];       
DT = [p];
DT = repmat(DT, 1, 5);
ConstructSeq(T,DT)
...
% pi back 0-\pm1 on spectator group if spec starts in 0
% if spec starts in \pm1, do not pi back
gmSEQ.CHN(numel(gmSEQ.CHN)+1).PBN=PBDictionary('MWSwitch2');
gmSEQ.CHN(numel(gmSEQ.CHN)).NRise=3;  
T=[t_R+i+t_T1-u-p2, u+i*3+t_R*2+t_T1-u-p2, u+i*3+t_R*2+d];
DT = [p2];
DT = repmat(DT, 1, 3);
ConstructSeq(T,DT)

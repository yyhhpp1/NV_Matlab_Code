...
T_AfterLaser = 5000;
T_AfterPulse = 2000;
T_initial_wait = T_AfterLaser + gmSEQ.pi*gmSEQ.misc*3 + T_AfterPulse;
...
gmSEQ.CHN(numel(gmSEQ.CHN)).NRise = 1;
gmSEQ.CHN(numel(gmSEQ.CHN)).T = T_initial_wait + gmSEQ.readout + T_AfterLaser - 700;
...
uploadSimplePulse('+X', gSG.FPGAFreq6, gain, gmSEQ.pi, 0)
seq1 = {['loop(' num2str(gmSEQ.misc) ', [+X])']};
ch = complieCHN({{0}, seq1});
uploadSimpleProg('f_PiCali_ch6', ch);

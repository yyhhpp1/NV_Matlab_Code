...
T=[...
   ...
   t_P1 + t_Expt*0, t_P2 + t_Expt*0, t_Pr + t_Expt*0, ...     -1,-1
   t_P1 + t_Expt*1,                  t_Pr + t_Expt*1, ...     -1,+1
                    t_P2 + t_Expt*2, t_Pr + t_Expt*2, ...     +1,-1
                                     t_Pr + t_Expt*3  ...     +1,+1
   ];
gmSEQ.CHN(numel(gmSEQ.CHN)).T = T;
gmSEQ.CHN(numel(gmSEQ.CHN)).DT = repmat([p_sg1], 1, 8);
...
T=[...
   ...
   t_P2 + t_Expt*1, ...     -1,+1
   t_P1 + t_Expt*2, ...     +1,-1
   t_P1 + t_Expt*3, t_P2 + t_Expt*3  ...     +1,+1
   ];
gmSEQ.CHN(numel(gmSEQ.CHN)).T = T;
gmSEQ.CHN(numel(gmSEQ.CHN)).DT = repmat([p_sg2], 1, 4);

gmSEQ.CHN(numel(gmSEQ.CHN)+1).PBN=PBDictionary('dummy1');
gmSEQ.CHN(numel(gmSEQ.CHN)).NRise=2;
T=[0, t_R*8+t_T1*4+i*11-20];
DT=[20 20];
ConstructSeq(T,DT)

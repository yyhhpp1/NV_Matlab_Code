...
gmSEQ.CHN(numel(gmSEQ.CHN)).NRise=27;
T=[...
   t_R, t_T1, t_R,          ...     
   t_R, t_T1, t_R,          ...     
   t_R, t_T1, t_R,          ...
...
T=[...
   ...
   t_P1 + t_Expt*4, t_P2 + t_Expt*4, t_Pr + t_Expt*4, ...     -1,-1
   t_P1 + t_Expt*5,                  t_Pr + t_Expt*5, ...     -1,+1
                                     t_Pr + t_Expt*6, ...     +1,0
                    t_P2 + t_Expt*7, t_Pr + t_Expt*7, ...     +1,-1
                                     t_Pr + t_Expt*8  ...     +1,+1
   ];
gmSEQ.CHN(numel(gmSEQ.CHN)).T = T;
gmSEQ.CHN(numel(gmSEQ.CHN)).DT = repmat([p_sg1], 1, 15);
...
T=[...
   ...
   t_P1 + t_Expt*6, ...     +1,0
   t_P1 + t_Expt*7, ...     +1,-1
   t_P1 + t_Expt*8, t_P2 + t_Expt*8  ...     +1,+1
   ];
gmSEQ.CHN(numel(gmSEQ.CHN)).T = T;
gmSEQ.CHN(numel(gmSEQ.CHN)).DT = repmat([p_sg2], 1, 6);

gmSEQ.CHN(numel(gmSEQ.CHN)+1).PBN=PBDictionary('dummy1');
gmSEQ.CHN(numel(gmSEQ.CHN)).NRise=2;
T=[0, t_R*17+t_T1*9+i*27-20];
DT=[20 20];
ConstructSeq(T,DT)
